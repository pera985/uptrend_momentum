...
Scores established uptrends (0-100 points) across 5 categories:
...
class StockScorer:
    """Score stocks on 100-point scale across 5 categories"""
    ...
    def score_trend_strength(self, df: pd.DataFrame) -> Tuple[float, Dict]:
        ...
        - ADX level (0-10 pts)
        - MA20 slope (0-10 pts)
        - Days in uptrend (0-5 pts)
        """
        latest = df.iloc[-1]
        details = {}
        score = 0

        # ADX Level (0-10 points)
        adx = latest['adx']
        if adx > 40:
            adx_score = 10
        elif adx > 30:
    ...
    def score_price_structure(self, df: pd.DataFrame) -> Tuple[float, Dict]:
        """
        Score price structure (0-20 points)

        Components:
        - Support quality (0-10 pts)
        - Pullback behavior (0-10 pts)
        """
        latest = df.iloc[-1]
    ...
    def score_risk_reward(self, df: pd.DataFrame) -> Tuple[float, Dict]:
        """
        Score risk/reward setup (0-15 or 0-20 points depending on config)

        Components:
        - Distance from MA20 (0-10 pts if volatility scoring enabled, else 0-8 pts)
        - Proximity to resistance (0-5 pts if volatility scoring enabled, else 0-7 pts)
        - Volatility quality (0-5 pts, only if ENABLE_VOLATILITY_IN_SCORING = True)
        """
        import config
        latest = df.iloc[-1]
        recent_60 = df.iloc[-60:]
        ...
        # Check if volatility scoring is enabled
        enable_vol_scoring = getattr(config, 'ENABLE_VOLATILITY_IN_SCORING', False)

        if enable_vol_scoring:
            # Adjusted scoring to fit 20 points total
            # Distance from MA20 (0-10 points)
            ...
            # Proximity to resistance (0-5 points)
            ...
            # OPTION 2: Volatility quality (0-5 points)
            volatility_20 = latest['volatility_20']

            if volatility_20 < 15:  # Very low vol
                vol_score = 5
            elif volatility_20 < 25:  # Low vol (smooth)
                vol_score = 5
            elif volatility_20 < 40:  # Moderate vol
                vol_score = 3
            else:  # High vol (>40%)
                vol_score = 0

            details['volatility_score'] = vol_score
            details['volatility_20'] = volatility_20
            score += vol_score

        else:
            # Original scoring (0-15 points total)
            # Distance from MA20 (0-8 points)
            distance_from_ma20 = ((latest['close'] - latest['ma_20']) / latest['ma_20']) * 100
            ...
            # Proximity to resistance (0-7 points)
            recent_high = recent_60['high'].max()
            room_to_resistance = ((recent_high - latest['close']) / latest['close']) * 100
    ...
    def calculate_total_score(self, df: pd.DataFrame) -> Tuple[float, Dict]:
        ...
        total = trend_score + momentum_score + volume_score + \
                structure_score + risk_reward_score
...
class UptrendScanner:
    ...
    def scan_market(self, exchanges: Optional[List[str]] = None,
        ...
        tickers = self.api.get_all_tickers(exchange=exchanges)
        ...
        # Sort established by score
        established_uptrends.sort(key=lambda x: x.get('score', 0), reverse=True)
        # Sort all scanned stocks by score as well
    ...
    def export_to_csv(self, results: Dict, output_dir: str = './output', strategy_id: str = None):
        ...
        # Create subdirectories for uptrend CSVs (no timestamp in folder names)
        early_csv_dir = f"{output_dir}/uptrends/csv/early"
        established_csv_dir = f"{output_dir}/uptrends/csv/established"
        all_stocks_csv_dir = f"{output_dir}/all_scanned/csv"  # No timestamp subdirectory
        ...
        # ====================================================================
        # EXPORT UPTRENDS (to ./output/uptrends/csv/early/ and established/)
        # ====================================================================
        ...
        # ====================================================================
        # EXPORT ALL SCANNED STOCKS (to ./output/all_scanned/csv/)
        # ====================================================================
    ...
    def plot_stock_chart(self, ticker: str, output_dir: str = './output/charts', strategy_id: str = None) -> Optional[str]:
        """
        ...
        # Get stock data (request 365 days to ensure we get at least 200 trading days)
        df = self.api.get_aggregates(ticker, days=365)
        if df is None or len(df) < 200:
        ...
        # Create figure with subplots - new compact vertical layout
        fig = plt.figure(figsize=(16, 14))
        # Height ratios: Price (2.5), Volume (0.7), Volatility (0.7), RSI (0.7), MACD (0.7), ADX (0.7), Summary (1.0)
        gs = GridSpec(7, 1, figure=fig, height_ratios=[2.5, 0.7, 0.7, 0.7, 0.7, 0.7, 1.0], hspace=0.0)

        # Main price chart (keep dimensions as-is, add 4 minor ticks)
        ax1 = fig.add_subplot(gs[0])
        ax1.plot(df.index, df['close'], label='Close', color='black', linewidth=1.5, antialiased=True)
        ax1.plot(df.index, df['ma_20'], label='MA20', color='blue', linewidth=1, alpha=0.7, antialiased=True)
        ax1.plot(df.index, df['ma_50'], label='MA50', color='orange', linewidth=1, alpha=0.7, antialiased=True)
        ax1.plot(df.index, df['ma_200'], label='MA200', color='red', linewidth=1, alpha=0.7, antialiased=True)

        # Bollinger Bands - draw boundary lines and fill
        ax1.plot(df.index, df['bb_upper'], label='BB Upper', color='gray', linewidth=1, alpha=0.5, linestyle='--', antialiased=True)
        ax1.plot(df.index, df['bb_middle'], label='BB Middle', color='gray', linewidth=1, alpha=0.4, linestyle=':', antialiased=True)
        ax1.plot(df.index, df['bb_lower'], label='BB Lower', color='gray', linewidth=1, alpha=0.5, linestyle='--', antialiased=True)
        ax1.fill_between(df.index, df['bb_lower'], df['bb_upper'], alpha=0.2, color='gray')

        ax1.set_title(f"{ticker} - Uptrend Analysis", fontsize=16, fontweight='bold')
        ...
        ax1.tick_params(axis='x', which='major', labeltop=False, labelbottom=False)  # Hide date labels on price chart
        ...
        try:
            ...
            if earnings_dates:
                # Convert earnings dates to datetime and filter to chart range
                earnings_dt = [pd.to_datetime(d) for d in earnings_dates]
                chart_start = df.index[0]
                chart_end = df.index[-1]
                earnings_in_range = [d for d in earnings_dt if chart_start <= d <= chart_end]

                # Add 'E' markers at bottom of price chart
                y_min, y_max = ax1.get_ylim()
                marker_y = y_min + (y_max - y_min) * 0.05  # 5% from bottom

                for earn_date in earnings_in_range:
                    ax1.text(earn_date, marker_y, 'E', fontsize=10, fontweight='bold',
        ...
        # Volume (no date labels on x-axis, shares x-axis with price)
        ...
        vol_line = ax2.plot(df.index, df['volume_ma_50'], label='Volume MA50', color='blue', linewidth=1, alpha=0.7, antialiased=True)
        ax2.set_ylabel('Volume', fontsize=10)
        ax2.grid(True, alpha=0.6)  # Increased grid opacity
        ax2.tick_params(axis='x', which='major', direction='in', length=8)  # Larger major ticks inside
        ax2.tick_params(axis='x', which='minor', direction='in', length=5)  # Larger minor ticks inside
        plt.setp(ax2.get_xticklabels(), visible=False)  # Remove date labels

        # Add secondary y-axis for price EMAs (right side)
        ax2_price = ax2.twinx()
        ...
        ema5_line = ax2_price.plot(df.index, price_ema5, label='EMA5', color='orange', linewidth=1.5, alpha=0.8, antialiased=True)
        ema20_line = ax2_price.plot(df.index, price_ema20, label='EMA20', color='purple', linewidth=1.5, alpha=0.8, antialiased=True)
        ...
        ax_vol.plot(df.index, df['volatility_20'], label='20-day Volatility', color='darkred', linewidth=1.5, antialiased=True)
        ax_vol.axhline(y=30, color='orange', linestyle='--', alpha=0.5, label='Moderate (30%)', antialiased=True)
        ax_vol.axhline(y=50, color='red', linestyle='--', alpha=0.5, label='High (50%)', antialiased=True)
        ax_vol.set_ylabel('Volatility (%)', fontsize=8)
        ax_vol.legend(loc='upper left', fontsize=8)
        ax_vol.grid(True, alpha=0.6)  # Increased grid opacity
        ax_vol.tick_params(axis='y', labelsize=8)
        ax_vol.tick_params(axis='x', which='major', direction='in', length=8)
        ax_vol.tick_params(axis='x', which='minor', direction='in', length=5)
        plt.setp(ax_vol.get_xticklabels(), visible=False)  # Remove date labels

        # RSI (directly below volatility, no space, smaller y-axis labels)
        ax3 = fig.add_subplot(gs[3], sharex=ax1)
        ax3.plot(df.index, df['rsi'], label='RSI', color='purple', linewidth=1.5, antialiased=True)
        ax3.axhline(y=70, color='red', linestyle='--', alpha=0.5, label='Overbought', antialiased=True)
        ax3.axhline(y=50, color='gray', linestyle='--', alpha=0.5, antialiased=True)
        ax3.axhline(y=30, color='green', linestyle='--', alpha=0.5, label='Oversold', antialiased=True)
        ax3.set_ylabel('RSI', fontsize=8)  # Smaller y-axis label
        ax3.set_ylim(0, 100)
        ax3.legend(loc='upper left', fontsize=8)
        ax3.grid(True, alpha=0.6)  # Increased grid opacity
        ax3.tick_params(axis='y', labelsize=8)  # Smaller y-axis tick labels
        ax3.tick_params(axis='x', which='major', direction='in', length=8)  # Larger major ticks inside
        ax3.tick_params(axis='x', which='minor', direction='in', length=5)  # Larger minor ticks inside
        plt.setp(ax3.get_xticklabels(), visible=False)  # Remove date labels

        # MACD (directly below RSI, no space, smaller y-axis labels)
        ax4 = fig.add_subplot(gs[4], sharex=ax1)
        ax4.plot(df.index, df['macd'], label='MACD', color='blue', linewidth=1.5, antialiased=True)
        ax4.plot(df.index, df['macd_signal'], label='Signal', color='red', linewidth=1.5, antialiased=True)
        ax4.bar(df.index, df['macd_histogram'], label='Histogram', alpha=0.3, color='gray')
        ax4.axhline(y=0, color='black', linestyle='-', alpha=0.3, antialiased=True)
        ax4.set_ylabel('MACD', fontsize=8)  # Smaller y-axis label
        ax4.legend(loc='upper left', fontsize=8)
        ax4.grid(True, alpha=0.6)  # Increased grid opacity
        ax4.tick_params(axis='y', labelsize=8)  # Smaller y-axis tick labels
        ax4.tick_params(axis='x', which='major', direction='in', length=8)  # Larger major ticks inside
        ax4.tick_params(axis='x', which='minor', direction='in', length=5)  # Larger minor ticks inside
        plt.setp(ax4.get_xticklabels(), visible=False)  # Remove date labels

        # ADX (directly below MACD, no space, smaller y-axis labels, SHOWS DATE)
        ax5 = fig.add_subplot(gs[5], sharex=ax1)
        ax5.plot(df.index, df['adx'], label='ADX', color='orange', linewidth=1.5, antialiased=True)
        ax5.axhline(y=25, color='green', linestyle='--', alpha=0.5, label='Strong Trend', antialiased=True)
        ax5.axhline(y=40, color='red', linestyle='--', alpha=0.5, label='Very Strong', antialiased=True)
        ax5.set_ylabel('ADX', fontsize=8)  # Smaller y-axis label
        ax5.legend(loc='upper left', fontsize=8)
        ax5.grid(True, alpha=0.6)  # Increased grid opacity
        ax5.tick_params(axis='y', labelsize=8)  # Smaller y-axis tick labels
        ax5.xaxis.set_major_formatter(mdates.DateFormatter('%y-%m-%d'))
        # Add 3 minor ticks between major ticks on ADX x-axis
        ax5.xaxis.set_minor_locator(AutoMinorLocator(4))  # 3 minor ticks between major ticks (n=4)
        ax5.tick_params(axis='x', which='major', direction='inout', length=10)  # Larger major ticks crossing
        ax5.tick_params(axis='x', which='minor', direction='inout', length=7)  # Larger minor ticks crossing
        ax5.tick_params(axis='x', rotation=45, labelsize=9)
        plt.setp(ax5.get_xticklabels(), visible=True)  # Show date labels on ADX only

        # Analysis Summary Panel (below ADX, matches ADX width)
        ...
        # Build summary text with multi-column format (integer scores, include RSI/ADX/Volatility/Float)
        info_text = f"ANALYSIS SUMMARY\n{'='*170}\n\n"
        info_text += f"Price: ${result['current_price']:.2f}  |  Score: {int(result['score'])}/100  |  Tier: {result['tier']}  |  RSI: {rsi_value:.1f}  |  ADX: {adx_value:.1f}  |  Volatility(20d): {vol_20_value:.1f}%  |  Float: {float_millions:.1f}M\n\n"
        ...
        info_text += f"Price Structure: {int(breakdown['price_structure'])}/20  |  Risk/Reward: {int(breakdown['risk_reward'])}/15  |  Days Above SMA20: {days_in_uptrend}  |  Eff. Volume: {effective_vol_pct:.2f}%\n"

        # Position text to span wider (increased separator from 120 to 150)
        # Use figure coordinates instead of axes coordinates for exact 16" width
        ax6.text(0.0, 0.95, info_text, transform=ax6.transAxes,
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        strategy_suffix = f"_{strategy_id}" if strategy_id else ""
        chart_file = f"{output_dir}/{ticker}{strategy_suffix}_{timestamp}.png"
    ...
    def plot_watchlist(self, stocks: List[Dict], output_dir: str = './output/charts', strategy_id: str = None) -> List[str]:
        ...
        for i, stock in enumerate(stocks, 1):
            ...
            chart_file = self.plot_stock_chart(ticker, output_dir, strategy_id)
            if chart_file:
